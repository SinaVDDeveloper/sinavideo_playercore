...
/**
 * Displays a video file. The TextureVideoView class can load images from
 * various sources (such as resources or content providers), takes care of
 * computing its measurement from the video so that it can be used in any layout
 * manager, and provides various display options such as scaling and tinting.
 * 
 * <em>Note: VideoView does not retain its full state when going into the
 * background.</em> In particular, it does not restore the current play state,
 * play position or selected tracks. Applications should save and restore these
 * on their own in {@link android.app.Activity#onSaveInstanceState} and
 ...
 * <p>
 * 
 * This code is based on the official Android sources for 4.4.2_r3 with the
 * following differences:
 * <ol>
 * <li>extends {@link android.view.TextureView} instead of a
 * {@link android.view.SurfaceView} allowing proper view animations</li>
 * <li>removes code that uses hidden APIs and thus is not available</li>
 * </ol>
 */
@SuppressLint("NewApi")
public class TextureVideoView extends TextureView implements ISinaVideoView {
...
//            openVideo();
			...
			if (mSurface != null) { // /create first time
				mSurface = new Surface(surface);
	...
	private void openVideo() {
		...
		try {
			...
			log("openVideo() mSurfaceHolder=[" + mSurface
					+ "],mSurfaceHolder.getSurface()=["
					+ mSurface + "]");
			mMediaPlayer.setSurface(mSurface); // mMediaPlayer.setSurfaceView(this);
			// mMediaPlayer
